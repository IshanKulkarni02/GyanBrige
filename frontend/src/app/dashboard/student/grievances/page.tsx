      ...
      <aside className="fixed left-0 top-0 bottom-0 w-64 glass border-r border-white/10 p-6 hidden lg:block">
        ...
        <nav className="space-y-2">
          {[
            ...
            { icon: '📢', label: 'Grievances', href: '/dashboard/student/grievances', active: true },
          ].map(item => (
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="glass rounded-2xl p-6 w-full max-w-lg">
              ...
              <div className="space-y-3">
                ...
                <textarea className="input-glass w-full h-28 resize-none" placeholder="Describe your grievance in detail…" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            ...
            {grievances.map((g, i) => (
              <button key={g.id} onClick={() => setSelected(g)}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-white/30 text-xs mt-1">{g.category} · {g.priority} · {new Date(g.createdAt).toLocaleDateString()}</p>
                </div>
