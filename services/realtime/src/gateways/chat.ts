  ...
  io.on('connection', (socket) => {
    ...
    socket.on('chat:join', async (roomId: string) => {
      try {
        ...
        // Validate membership — memberIds may be absent on legacy class rooms
        ...
        socket.emit('chat:error', { code: 'JOIN_FAILED', message: (e as Error).message });
      }
    });

    socket.on('chat:send', async (msg: { roomId: string; kind: string; body?: string; attachments?: unknown[] }) => {
      try {
        // Verify caller is still a member before writing
        const room = await rooms.findOne({ _id: msg.roomId as unknown as never });
        if (!room) return socket.emit('chat:error', { code: 'ROOM_NOT_FOUND' });
        if (Array.isArray(room.memberIds) && !room.memberIds.includes(userId)) {
          return socket.emit('chat:error', { code: 'FORBIDDEN' });
        }
        ...
        socket.emit('chat:error', { code: 'SEND_FAILED', message: (e as Error).message });
      }
    });
    ...
    socket.on('chat:read', async ({ roomId, messageId }: { roomId: string; messageId: string }) => {
      try {
        ...
        // read receipts are best-effort — don't surface to user
      }
    });
  });
}
